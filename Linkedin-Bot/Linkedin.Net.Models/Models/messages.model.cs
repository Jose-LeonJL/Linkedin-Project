...
namespace Linkedin.Net.Models
{
    public class messages
    {
        ...
        public static messages SelectByMessage(database database, string message)
        {
            using (var db = database.connection)
            {
                var sql = $"select * from messages where message=@message";
                return db.QueryFirstOrDefault<messages>(sql, new { message = message });
            }
        }
