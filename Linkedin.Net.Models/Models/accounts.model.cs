...
namespace Linkedin.Net.Models
{
    public class accounts
    {
        ...
        public static accounts SelectByEmail(database database, string Email)
        {
            using (var db = database.connection)
            {
                var sql = $"select * from accounts where account_mail=@Email";
                return db.QueryFirstOrDefault<accounts>(sql, new { Email = Email });
            }
        }
        ...
        public static List<accounts> SelectAccountsAviable(database database)
        {
            using (var db = database.connection)
            {
                var sql = "SELECT * FROM accounts where account_banned = false;";
                var result = db.Query<accounts>(sql).ToList();
                return result;
            }
        }
        public static List<accounts> SelectAccountsWithProxyBanned(database database)
        {
            using (var db = database.connection)
            {
                var sql = "SELECT ac.account_id,ac.account_mail_server,ac.account_name," +
                    "ac.account_mail,ac.account_pass,ac.account_linkedin_pass,ac.account_mobile_number," +
                    "ac.account_profile_url,ac.account_banned,ac.account_salenav,ac.account_selected," +
                    "ac.account_selected,ac.account_cookie,ac.create_at " +
                    ...
                    "WHERE px.proxy_banned = true and ac.account_banned = false; ";
                var result = db.Query<accounts>(sql).ToList();
                return result;
            }
        }
